(() => {
    ...
    if (spollersArray.length > 0) {
        ...
        function setSpollerAction(e) {
            const el = e.target;
            if (el.closest("[data-spoller]") && el.closest("[data-spollers]").classList.contains("_init")) {
    ...
    core.use([ Navigation, Pagination, Parallax ]);
    let sliders = document.querySelectorAll("._swiper");
    if (sliders) for (let index = 0; index < sliders.length; index++) {
    ...
    function initSliders() {
        if (document.querySelector(".slider-hero__body")) new core(".slider-hero__body", {
            observer: true,
            observeParents: true,
            slidesPerView: 1,
            spaceBetween: 32,
        ...
        if (document.querySelector(".slider-rooms__body")) new core(".slider-rooms__body", {
            ...
            on: {
                slideNextTransitionStart: function() {
                    const prevArrow = document.querySelector(".slider-rooms .slider-arrow_prev");
                    prevArrow.classList.add("_show");
                }
            }
        });
    ...
    function formEmailValid() {
        ...
        const formItem = form.querySelector("input");
        form.addEventListener("submit", formSend);
        async function formSend(e) {
            e.preventDefault();
            let error = validInput(formItem);
            if (error === 0) {
    ...
    function formAddError(formItem) {
        const form = formItem.closest("form");
        form.classList.add("_error");
    }
    function formRemoveError(formItem) {
        const form = formItem.closest("form");
        form.classList.remove("_error");
    }
    function emailTest(formItem) {
    ...
    function functions_removeClasses(array, className) {
        for (const item of array) item.classList.remove(className);
    }
    const pageLockToggle = () => {
...
***************************************************************************** */    var __assign = function() {
    ...
    function furnitureGalletyMove() {
        const furniture = document.querySelector(".furniture__body");
        if (furniture && !isMobile.any()) {
            const furnitureItems = document.querySelector(".gallery-furniture");
            const furnitureColums = document.querySelectorAll(".gallery-furniture__column");
            const speed = furniture.dataset.speed;
            let coordXprocent = 0;
            let positionX = 0;
            function setMouseGalleryStyle() {
                let furnitureColumsWidth = 0;
                furnitureColums.forEach((el => {
                    furnitureColumsWidth += el.offsetWidth;
                }));
                if (furnitureColumsWidth > furniture.offsetWidth) {
                    const furnitureDifferent = furnitureColumsWidth - furniture.offsetWidth;
                    ...
                    furnitureItems.style.transform = `translate3d(0px,0,0)`;
                }
            }
    ...
    furnitureGalletyMove();
    function toggleMenu(close) {
        const page = document.documentElement;
        if (!page.classList.contains("menu-slide")) {
            if (close) {
                pageLockToggle();
                page.classList.remove("menu-open");
                return;
            }
            page.classList.add("menu-slide");
            pageLockToggle();
            page.classList.toggle("menu-open");
            setTimeout((() => {
                page.classList.remove("menu-slide");
            }), 300);
        }
    }
    function headerScroll() {
        ...
        if (header) {
            const callback = function(entries, observer) {
                if (entries[0].isIntersecting) header.classList.remove("_scroll"); else header.classList.add("_scroll");
            };
    ...
    function popup() {
        ...
        if (!popup.classList.contains("opening")) {
            popup.classList.add("_show");
            popup.classList.add("opening");
            pageLockToggle();
            document.addEventListener("keydown", handleKeyDown);
            if (!popup.querySelectorAll(".cart-list__item").length) popup.classList.add("_empty"); else popup.classList.remove("_empty");
        }
    }
    ...
    function addToCart(productButton, productId) {
        if (!productButton.classList.contains("_hold")) {
            ...
            const productButtonText = productButton.textContent;
            productButton.textContent = "Adding to cart...";
            ...
            const flyImageWidth = productImage.offsetWidth;
            const flyImageHeight = productImage.offsetHeight;
            const flyImageTop = productImage.getBoundingClientRect().top;
            const flyImageLeft = productImage.getBoundingClientRect().left;
            flyImage.setAttribute("class", "fly-image");
            ...
            flyImage.addEventListener("transitionend", (() => {
                if (productButton.classList.contains("_fly")) {
                    flyImage.remove();
                    updateCart(productButton, productId, true, productButtonText);
                    productButton.classList.remove("_fly");
                }
            }));
        }
    }
    function updateCart(productButton, productId, productAdd = true, productButtonText = "") {
        const cart = document.querySelector("[data-cart-popup]");
        ...
        const productPrice = product.querySelector(".product-price span").innerHTML;
        if (productAdd) {
            ...
            if (!cartProduct) {
                ...
                const productTemplate = `\n\t\t\t<li data-cart-pid="${productId}" class="cart-list__item">\n\t\t\t<div class="cart-list__content">\n\t\t\t<a href="" class="cart-list__img">${productImage}</a>\n\t\t\t<div class="cart-list__info">\n\t\t\t<a href="" class="cart-list__title">${productTitle}</a>\n\t\t\t<div href="" class="cart-list__price">${productPrice}</div>\n\t\t\t<button type="button" class="cart-list__delete">Remove</button>\n\t\t\t</div>\n\t\t\t</div>\n\t\t\t<input type="number" value="1" name="quantity" class="cart-list__quantity-input" >\n\t\t\t</li>\n\t\t\t`;
                cartList.insertAdjacentHTML("beforeend", productTemplate);
            } else {
                const cartListQuantityInput = cartProduct.querySelector(".cart-list__quantity-input");
                cartListQuantityInput.value = ++cartListQuantityInput.value;
            }
            cartSubtotal.textContent = getNewSubtotal(cartSubtotal.innerHTML, productPrice);
            productButton.classList.remove("_hold");
            productButton.textContent = productButtonText;
            cartPopup.classList.remove("_empty");
            popup();
        } else {
            const cartListQuantityInput = cartProduct.querySelector(".cart-list__quantity-input");
            cartSubtotal.textContent = getNewSubtotal(cartSubtotal.innerHTML, productPrice, false);
            if (parseInt(cartListQuantityInput.value) !== 0) cartListQuantityInput.value = --cartListQuantityInput.value;
            if (!parseInt(cartListQuantityInput.value)) cartProduct.remove();
        ...
        if (!cart.classList.contains("_empty")) cartList.addEventListener("change", changeQuantity); else cartList.removeEventListener("change", changeQuantity);
    }
    function changeQuantity(e) {
        const targetElement = e.target;
        console.log("я працюю!");
        if (targetElement.closest(".cart-list__quantity-input")) if (targetElement.value !== "0") {
            const cartIcon = document.querySelector(".cart-header__icon");
            const cartQuantity = cartIcon.querySelector("span");
            const products = document.querySelectorAll(".cart-list li");
            const cartSubtotal = document.querySelector(".cart__subtotal");
            let allProductsPrice = 0;
            let allQuantity = 0;
            for (const product of products) {
                const price = product.querySelector(".cart-list__price").innerHTML;
                const quantity = product.querySelector(".cart-list__quantity-input").value;
                const productPrice = +price.replace(/[^0-9]/g, "") * +quantity;
                allProductsPrice += productPrice;
                allQuantity += +quantity;
            }
            allProductsPrice = `Rp ${allProductsPrice.toLocaleString("id-ID")}`;
            cartSubtotal.textContent = allProductsPrice;
    ...
    function getNewSubtotal(currentSubtotal, productPrice, add = true) {
        const currentSubtotalNumber = +currentSubtotal.replace(/[^0-9]/g, "");
        const productPriceNumber = +productPrice.replace(/[^0-9]/g, "");
        let newSubtotal;
        if (add) newSubtotal = currentSubtotalNumber + productPriceNumber; else newSubtotal = currentSubtotalNumber - productPriceNumber;
        return `Rp ${newSubtotal.toLocaleString("id-ID")}`;
    }
    ...
    function documentActions(e) {
        ...
        if (window.innerWidth > 992 && isMobile.any()) {
            if (targetElement.closest(".dropdown-menu__text")) targetElement.closest(".dropdown-menu").classList.toggle("_hover");
            if (!targetElement.closest(".dropdown-menu") && document.querySelectorAll(".dropdown-menu._hover").length > 0) functions_removeClasses(document.querySelectorAll(".dropdown-menu._hover"), "_hover");
        }
        if (targetElement.closest(".icon-menu")) toggleMenu(); else if (!targetElement.closest(".menu") && document.querySelector(".menu-open")) toggleMenu(close);
        if (targetElement.closest(".search-header__icon")) {
            if (!targetElement.classList.contains("_slide")) {
                targetElement.classList.add("_slide");
                document.querySelector(".search-header").classList.toggle("_active");
                setTimeout((() => {
                    targetElement.classList.remove("_slide");
                }), 300);
            }
        } else if (!targetElement.closest(".search-header") && document.querySelector(".search-header._active")) document.querySelector(".search-header").classList.remove("_active");
        if (targetElement.closest(".product-cover__btn")) {
            const productId = targetElement.closest(".item-product").dataset.pid;
            addToCart(targetElement, productId);
            e.preventDefault();
        }
        if (targetElement.closest("[data-cart]")) popup(); else if (!targetElement.closest("[data-popup-body]") && targetElement.closest("[data-cart-popup]") || targetElement.closest("[data-popup-close]")) {
            const popup = document.querySelector("[data-cart-popup]");
            closePopup(popup);
        }
        ...
        if (targetElement.closest(".empty-cart__btn")) {
            const popup = document.querySelector("[data-cart-popup]");
            closePopup(popup);
        }
